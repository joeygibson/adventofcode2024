(declaim (optimize (speed 3) (safety 0) (debug 0) (space 2)))
...
(defun find-chunks (map)
  (let* ((chunks nil)
    ...
    (make-array (length chunks) :initial-contents (reverse chunks))))
...
(defun compact-2 (map)
  (let* ((file-chunks (partition-array (find-chunks map)
                                       (lambda (chunk)
                                         (not (equal (caar chunk) "."))))))
    (format t "~&chunks: ~a~%" (length file-chunks))
    (loop for chunk across (reverse file-chunks)
          do (progn
               (print (caar chunk))
               (let* ((chunks (find-chunks map)))
                 (multiple-value-bind (_ empty-chunks)
                     (partition-array chunks (lambda (chunk)
                                               (not (equal (caar chunk) "."))))
                   (when-let ((empty (loop for chnk across empty-chunks
                                           if (>= (chunk-len chnk)
                                                 (chunk-len chunk))
                                             return chnk)))
                     (when (< (cddr empty)
